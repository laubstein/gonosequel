...
make test           # unit + integration tests, starts/stops MongoDB and Redis via Docker itself
make test-short      # unit tests only, no Docker required
```

Every backend change that touches `pkg/client` (MongoDB), `pkg/redis`, or `pkg/api` should
have integration test coverage against a real server (via `testcontainers-go`, already wired
into `TestMain` in all three packages) — this project has caught real bugs that unit tests
with mocks would have missed, precisely because they only surface against a live server.
